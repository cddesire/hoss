...
class ReduceTask extends Task {
	static {                                        // register a ctor
		WritableFactories.setFactory(ReduceTask.class,
		new WritableFactory() {
		...
		private void writeSkippedRec(KEY key, VALUE value) throws IOException {
			if (skipWriter == null) {
				...
				skipWriter = SequenceFile.createWriter(
				                 ...
				                 keyClass, valClass, CompressionType.BLOCK, reporter);
			}
	...
	class ReduceCopier<K, V> implements MRConstants {
		...
		public boolean fetchOutputs() throws IOException {
			...
			while (copiedMapOutputs.size() < numMaps && mergeThrowable == null) {
				...
				if (retryFetches.size() > 0) {
					LOG.info(reduceTask.getTaskID() + ": " +
					         "Got " + retryFetches.size() + " map-outputs from previous failures");
				}
					...
					if (cr.getSuccess()) {  // a successful copy
						...
						if (noFailedFetches == maxFetchFailuresBeforeReporting) {
							...
							boolean reducerProgressedEnough =
							    (((float)numCopied / numMaps) >= MIN_REQUIRED_PROGRESS_PERCENT);
							...
							int minShuffleRunDuration = (shuffleProgressDuration > maxMapRuntime)
							    ? shuffleProgressDuration
							    : maxMapRuntime;
							boolean reducerStalled = (((float)stallDuration / minShuffleRunDuration)
			...
			try {
				...
				LOG.info("getMapsEventsThread threw an exception: " + StringUtils.stringifyException(ie));
			}
