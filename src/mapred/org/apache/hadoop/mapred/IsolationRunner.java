...
public class IsolationRunner {
	...
	private static class FakeUmbilical implements TaskUmbilicalProtocol {
		...
		public void shuffleError(TaskAttemptID taskId, String msg, JvmContext jvmContext) throws IOException {
			LOG.info("Task " + taskId + " reporting shuffle error: " + msg);
		}
	...
	boolean run(String[] args) throws ClassNotFoundException, IOException, InterruptedException {
		...
		Path localMetaSplit = new LocalDirAllocator("mapred.local.dir").getLocalPathToRead(
		    TaskTracker.getLocalSplitFile(conf.getUser(), taskId.getJobID().toString(), taskId.toString()), conf);
